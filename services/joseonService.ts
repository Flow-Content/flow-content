// ─────────────────────────────────────────────────────────────────────────────
// JOSEON (야담 Hàn Quốc) — prompt Veo3 cho truyện dân gian Hàn thời Joseon.
//
// KHÁC tab 2 (Ukiyo-e Edo) ở CÁCH GIỮ NHÂN VẬT:
//   • Ukiyo-e nét phẳng, mặt na ná nhau → khóa bằng mô tả dán vào prompt là đủ.
//   • Manhwa Hàn mặt cá tính, tả bằng chữ bao nhiêu cũng trôi → phải đi đường
//     ẢNH THAM CHIẾU như tab 1: mỗi nhân vật một mã @Ten, prompt clip chỉ gọi
//     SUBJECT_@Ten, tuyệt đối không tả ngoại hình.
//
// Vì thế pha "khóa nhân vật" đổi thành XƯỞNG TẠO HÌNH:
//   1. Đọc truyện → tách nhân vật, chốt FACE DNA (nét mặt bất biến) + các GIAI ĐOẠN
//      (bé/lớn, nghèo/giàu, hạ nhân/đỗ đạt). Mỗi giai đoạn một mã @Ten riêng vì
//      ảnh tham chiếu của từng giai đoạn là ảnh khác nhau.
//   2. Sinh PROMPT TẠO HÌNH cho từng giai đoạn — FACE DNA dán NGUYÊN VĂN ở mọi
//      giai đoạn nên bé hay già, nghèo hay giàu vẫn nhận ra một khuôn mặt.
//   3. Sinh prompt clip: CODE chọn mã @ đúng giai đoạn theo cueId rồi mới đưa AI.
// Kịch bản đầu vào thường là TIẾNG HÀN; prompt xuất ra TIẾNG ANH.
// ─────────────────────────────────────────────────────────────────────────────
...
/** Style chốt của anh chủ cho PROMPT CLIP — sửa được ở ô "Hậu tố phong cách". */
export const JOSEON_DEFAULT_STYLE =
    'Traditional 2D Joseon Korean Asian folk-tale animation, Korean manhwa-inspired ink linework, ' +
    'clean bold black outlines, flat cel shading, subtle watercolor wash, paper grain, hand-painted textures, ' +
    'soft natural daylight, gentle shadows, calm rural atmosphere with subtle tension, ' +
    'muted earthy greens, straw gold, warm brown, pale cream, river blue, stone gray, ' +
    'moderate saturation, soft contrast, wide eye-level composition, 28mm lens equivalent, ' +
    'deep depth of field, layered cinematic framing, sharp focus, highly detailed, high-resolution animation still';

/** Style chốt của anh chủ cho PROMPT TẠO HÌNH nhân vật (nền trắng, tươi sáng). */
export const JOSEON_CHAR_STYLE =
    'Traditional 2D Joseon folk-tale animation, Korean manhwa-inspired linework, bold black outlines, ' +
    'crisp cel shading, storybook illustration style, bright and colorful children’s animation look, ' +
    'vivid saturated colors, strong contrast, clear readable character design, bold primary and secondary colors, ' +
    'cheerful palette, clean color blocking, crisp color separation, clean flat colors, smooth polished finish, ' +
    'minimal texture overlay, white background, eye-level wide composition, sharp focus, highly detailed, ' +
    'polished high-resolution animation still.';

/**
 * KHÓA ÂM THANH — Veo rất hay tự lồng giọng đọc/thoại khi prompt không chặn audio
 * (tham khảo tab Ukiyo-e: câu chặn nằm ngay trong style suffix nên không dính).
 * Code tự nối câu này vào CUỐI mọi prompt clip, NHƯNG chỉ khi hậu tố style của
 * người dùng chưa có câu chặn audio — tránh lặp 2 lần.
 */
export const VEO_AUDIO_LOCK =
    'Audio: only natural ambient sounds and environmental sound effects, absolutely no narration, ' +
    'no voiceover, no dialogue, no speech, no singing, no humming, no whispering, ' +
    'no human voice of any kind, no background music.';

/** Style đã tự chặn audio chưa? Chưa thì code mới nối VEO_AUDIO_LOCK. */
export const styleDaChanAudio = (style: string): boolean =>
    /no narration|no voiceover|no dialogue|no human voice/i.test(style);

/**
 * GHI CHÚ VĂN HÓA CHO BẢN ĐỒ PHÂN CẢNH — truyền vào generateSceneMap để câu
 * "setting" ra đúng hanok Joseon thay vì "a village at dusk" chung chung.
 */
export const JOSEON_SCENE_NOTE = `WORLD: rural Korea, Joseon dynasty. The input script is in KOREAN; write every setting sentence in ENGLISH.
Use concrete Joseon vocabulary, never generic Asian wording: thatched-roof hanok cottage, tiled-roof yangban manor, servants quarters wing around a packed-earth courtyard, raised wooden daecheong floor, hanji paper sliding doors, jangdokdae terrace of earthenware jars, mud-and-stone wall, wood-fired agungi kitchen, village school room, roadside tavern with a straw awning, terraced rice paddies, pine-covered hills, a stone-stepped stream, snow-covered mountain rocks, market road.
FORBIDDEN in every setting sentence: torii gates, Japanese stone lanterns, tatami, shoji screens, sakura-lined streets, Chinese red lanterns, curved Chinese pavilions, pagodas, temples of any other country, and anything modern.`;

/** Trang phục/tóc theo thân phận — dùng chung cho pha tạo hình & pha viết clip. */
const JOSEON_COSTUME_RULES = `- Đầy tớ / nông dân (머슴, 노비, 농부): coarse undyed hemp or cotton jacket and baggy trousers, faded grey-brown, cloth headband or bare topknot, straw sandals. TUYỆT ĐỐI KHÔNG có gat (nón đen) và không mặc lụa.
- Đàn bà hạ nhân (찬모, 여종): short jeogori + long chima in dull indigo/grey/brown, apron, low bun with a plain wooden pin, cloth head wrap when working.
- Quý tộc / chủ nhà (양반, 진사, 대감): flowing silk durumagi robe in white/jade/pale blue, black horsehair gat hat over a topknot with a manggeon headband, long thin pipe or folding fan, soft leather shoes.
- Quan lại / nha dịch / lính (관원, 나졸, 포교): dark blue or red official robe with a rank badge, black official hat, sash and belt; lính mang gậy đỏ-đen.
- Ngự sử vi hành (암행어사) lúc lộ diện: fine dark-blue silk official robe, black gat, small round brass mapae medallion.
- Bà đồng (무당): loud red-and-blue layered robe, tall crown-like headdress, bells and fan.
- Thầy đồ (훈장): plain white or beige scholar robe, black gat, thin beard.
- Trẻ em: bé trai chưa cưới để tóc tết đuôi dài buộc dải, áo màu tươi; bé gái tóc bím với dải daenggi đỏ. TUYỆT ĐỐI KHÔNG búi tóc sangtu, không đội gat.
- Đàn bà đã chồng: búi thấp cài trâm binyeo. Gái chưa chồng: bím dài dải đỏ. Đàn ông đã cưới: búi sangtu. Chưa cưới (총각) dù đã lớn tuổi: tóc buộc đuôi sau lưng — chi tiết này thường CHÍNH LÀ nút thắt của truyện, đọc kỹ.
...
- CẤM TUYỆT ĐỐI lẫn văn hóa khác: kimono, yukata, obi, geta, samurai topknot kiểu Nhật, hanfu, cheongsam/qipao, đuôi sam kiểu Thanh. Chỉ hanbok Joseon.`;

/** Một GIAI ĐOẠN của nhân vật — mỗi giai đoạn một ảnh tham chiếu, một mã @. */
export interface JoseonStage {
    /** dòng SRT bắt đầu áp dụng (1 = từ đầu truyện) */
    tuDong: number;
    /** mốc đổi, tiếng Việt ngắn (vd "6 tuổi, theo hầu đi học") */
    moc: string;
    /** mã gọi trong prompt clip, KHÔNG kèm @ (vd "Geumdong_be") */
    ma: string;
...
export interface JoseonCharacter {
    ten: string;
    vai: string;
    /** true = nhóm người (đám nô tì, dân làng, nha dịch) — KHÔNG vẽ ảnh, tả generic trong cảnh */
    laNhom?: boolean;
...
export const maTaiDong = (c: JoseonCharacter, cueId: number): string => {
...
export const tomTatTaiDong = (c: JoseonCharacter, cueId: number): string => {
...
// ─────────────────────────────────────────────────────────────────────────────
export const analyzeJoseonCharacters = async (args: {
    items: SubtitleItem[];
    model: string;
    signal?: AbortSignal;
}): Promise<JoseonCharacter[]> => {
    ...
    const systemText = `ROLE: Bạn là đạo diễn casting cho phim hoạt hình 2D truyện dân gian Hàn Quốc (야담) thời Joseon.

NHIỆM VỤ: đọc kịch bản (đánh số theo dòng phụ đề, thường là TIẾNG HÀN) rồi lập DANH SÁCH NHÂN VẬT kèm NÉT MẶT BẤT BIẾN và CÁC GIAI ĐOẠN của từng người. Mọi mô tả viết bằng TIẾNG ANH.

════════ CHỌN NHÂN VẬT ════════
- Tổng 8–12 mục. Gồm: nhân vật chính, nhân vật phụ xuất hiện nhiều, nhân vật đơn lẻ nhưng có vai trò rõ.
- NHÓM người xuất hiện cùng nhau (đám nô tì, dân làng, nha dịch, đám trẻ học chữ) → 1 mục với "laNhom": true. NHÓM KHÔNG VẼ ẢNH: mọi giai đoạn của nhóm để "ma": "" (chuỗi rỗng), và "tomTat" là cụm TIẾNG ANH generic ngắn để tả thẳng trong cảnh, ví dụ "a group of seven household servants in coarse grey hemp clothes". SỐ LƯỢNG lấy đúng con số truyện nêu; truyện không nêu thì ghi "a small group of". Nhóm chỉ cần 1 giai đoạn.
...
- TỪ 14 TUỔI TRỞ LÊN: lúc này mới tách giai đoạn thật — thiếu niên → trưởng thành → đổi thân phận (đỗ đạt, phóng thích, giàu lên) → già tóc bạc.
- NGƯỜI LỚN: chỉ tách khi thân phận hoặc trang phục đổi HẲN (hạ nhân→quan, nghèo→giàu, chưa vợ→có vợ nên đổi kiểu tóc, trung niên→già tóc bạc). Bẩn/sạch, ướt mưa, buồn/vui, mệt mỏi KHÔNG phải giai đoạn mới.
...
- "moc": lý do đổi, tiếng Việt ngắn (vd "tuổi nhỏ 4–10"; "10 năm sau, đỗ đạt làm quan").
- "ma": MÃ GỌI trong prompt — CHỈ chữ Latinh không dấu, số và gạch dưới, dạng TenNhanVat_ChangTruyen, ví dụ Geumdong_child, Geumdong_officer, Mansuk_old. Mỗi giai đoạn một mã DUY NHẤT trong toàn bộ danh sách. Riêng giai đoạn sơ sinh để rỗng "".
- "tomTat": một câu TIẾNG ANH ngắn (10–18 từ) nêu tuổi + thân phận + tóc/mũ + trang phục Joseon của giai đoạn đó.
Nhân vật không đổi ngoại hình thì chỉ 1 giai đoạn.

════════ TRANG PHỤC & TÓC PHẢI ĐÚNG THÂN PHẬN JOSEON ════════
${JOSEON_COSTUME_RULES}
...
JSON SAFETY: TUYỆT ĐỐI không dùng dấu nháy kép (") bên trong giá trị chuỗi.`;
    ...
    const list: JoseonCharacter[] = (Array.isArray(data?.characters) ? data.characters : [])
        .map((c: any, ci: number) => {
            ...
            const stages: JoseonStage[] = (Array.isArray(c.giaiDoan) ? c.giaiDoan : [])
                .map((g: any, gi: number) => {
                ...
                .sort((a: JoseonStage, b: JoseonStage) => a.tuDong - b.tuDong);
            return {
                ten,
        ...
        .filter((c: JoseonCharacter) => c.giaiDoan.length > 0);
...
// ─────────────────────────────────────────────────────────────────────────────
export const generateCharacterSheets = async (args: {
    characters: JoseonCharacter[];
    model: string;
    ...
    const systemText = `ROLE: Bạn là họa sĩ thiết kế tạo hình (character sheet) cho phim hoạt hình 2D truyện dân gian Hàn Quốc thời Joseon.
...
2. Tuổi + giới + thân phận Joseon (theo "tomTat").
3. Chuỗi faceDna NGUYÊN VĂN.
4. Tóc/mũ đúng thân phận và tình trạng hôn nhân.
5. Trang phục hanbok đúng thân phận: loại áo, màu, chất vải, giày/dép, phụ kiện đặc trưng.
...
════════ CẤM ════════
- CẤM viết tên nhân vật, mã, chữ Hàn hay bất kỳ chữ nào vào prompt (ảnh không được có chữ).
...
- CẤM lẫn văn hóa: kimono, obi, geta, hanfu, cheongsam, đuôi sam kiểu Thanh, đồ hiện đại.
...
// ─────────────────────────────────────────────────────────────────────────────
export interface JoseonBatchResponse {
    items: { key: string; prompt: string }[];
    usage: ChatResult['usage'];
}

export const generateJoseonPromptsBatch = async (args: {
    ...
    // (không nối trùng nếu style đã tự có câu chặn).
    ...
    const systemText = `ROLE: You are a director + storyboard artist writing Veo3 video prompts for a Korean Joseon-dynasty folk tale (야담) rendered as traditional 2D hand-painted animation. The narration script is in KOREAN; every prompt you write must be in ENGLISH.
...
════════ JOSEON WORLD LOCK (chốt cứng, không được lệch) ════════
- Kiến trúc: hanok mái rạ (nhà nghèo) hoặc mái ngói cong nhẹ (nhà quan), sân đất nện, sàn gỗ daecheong, cửa lùa giấy hanji, tường đá trộn bùn, dãy chum sành jangdokdae, bếp lò agungi, nhà kho thóc, trường làng seodang, quán rượu jumak mái rơm.
- Đồ vật: gùi khung gỗ jige, bàn ăn nhỏ soban, đèn dầu deungjan, chiếu rơm, thùng gỗ, cối đá, kiệu gama phủ vải, ngựa nhỏ, roi tre, bát đồng, chum gạo.
- Thiên nhiên: đồi thông, ruộng bậc thang, suối đá, tuyết phủ sườn núi, hoa mai trắng, quạ đen.
- CẤM TUYỆT ĐỐI: cổng torii, đèn đá Nhật, chiếu tatami, cửa shoji, kimono/geta, đèn lồng đỏ Trung Hoa, mái đao cong kiểu Tàu, chùa tháp nhiều tầng, và mọi đồ hiện đại (kính cửa sổ, dây điện, gạch nung đỏ).

════════ CÁCH VIẾT MỖI PROMPT ════════
1. INPUT: mảng JSON các clip { "key", "part" (vd 2/3 = pha 2 của 3 pha cùng một câu), "text" (khúc lời kể tiếng Hàn của clip này), "seconds" }.
...
10. TỪ CẤM — Veo vẽ thành MÁU và MẶT NỨT VỠ (lỗi đo thực tế 2026-07-31):
   ...
   · ragged / tattered / torn / threadbare / rags / half-naked / revealing / exposed skin khi tả trang phục → thay bằng "plain worn hemp clothes", "neatly patched faded jeogori". Áo quần NGHÈO nhưng NGUYÊN VẸN, KÍN ĐÁO — Veo vẽ áo rách là lộ cơ thể nhạy cảm. Nhân vật NỮ dù khổ vẫn tả xinh đẹp ưa nhìn.
11. KHÔNG dựng cảnh có chữ trong khung hình: không biển hiệu có chữ, không sách mở thấy chữ, không thư từ trải rộng, không câu đối treo tường. Cần tả chữ nghĩa thì quay góc thấy giấy nghiêng, tay cầm bút, cuộn giấy cuộn lại.
${customInstructions ? `\n════════ YÊU CẦU THÊM CỦA NGƯỜI DÙNG ════════\n${customInstructions}` : ''}
...
JSON SAFETY: TUYỆT ĐỐI không dùng dấu nháy kép (") bên trong prompt.`;
    ...
    // Hậu tố (style + khóa kỹ thuật) do CODE nối — AI khỏi viết lại mỗi clip.
